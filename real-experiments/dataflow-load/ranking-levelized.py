...
#micros0 = ["encode_20000","frame_20000","hightrain","inference","encode_20000"]#sys.argv[1].strip("][").split(",")
#print(type(micros0))

# Converting string to list
#num_of_src = 1
num_of_apps = len(sys.argv) - 3
lenn_app = 5 #max(len(micros0) , len(micros1))
#print(lenn_app)
#print(len(sys.argv))
...
resources = sys.argv[len(sys.argv)-2].strip("][").split(",")
num_of_src = int(sys.argv[len(sys.argv)-1])
...
for i in range(lenn_app):
	fileObject = open("D:\\00Research\\matching\\scheduler\\TO-Upload\\c3-match-main\\real-experiments\\dataflow-load\\MPL.yaml", 'w').close()
	capfile = open('D:\\00Research\\matching\\scheduler\\TO-Upload\\c3-match-main\\real-experiments\\dataflow-load\\capacities-testbed.yml', 'w').close()
	fileObject = open("D:\\00Research\\matching\\scheduler\\TO-Upload\\c3-match-main\\real-experiments\\dataflow-load\\DPL.yaml",'w').close()
	for k in range(num_of_apps):
		...
		for j in range(len(resources)):
			#print(resources[j])
			dictlistmicros[i][(resources[j],levels[k][i])] = Tm[k][i][j]+Tq[k][i][j]
		...
		#print((sorted_dictlistmicros[i]))
		#print(dict(sorted_dictlistmicros[i]).keys())
		...
		#print((listofkeys[0]))
		with open(r'D:\\00Research\\matching\\scheduler\\TO-Upload\\c3-match-main\\real-experiments\\dataflow-load\\MPL.yaml', 'a') as file:
	...
	for j in range(len(resources)):
				...
				dicttttt = {listofkeys[0]:listofvalues}
				#print((listofkeys[0]))
				with open(r'D:\\00Research\\matching\\scheduler\\TO-Upload\\c3-match-main\\real-experiments\\dataflow-load\\DPL.yaml', 'a') as file:
					documents = yaml.dump(dicttttt, file)
					file.close()
				####print(dict(sorted_dictlistResources[j]))
				####print()
				capacity_dict = {listofkeys[0]:5}
				with open(r'D:\\00Research\\matching\\scheduler\\TO-Upload\\c3-match-main\\real-experiments\\dataflow-load\\capacities-testbed.yml', 'a') as capfile:
					yaml.dump(capacity_dict, capfile)
					file.close()
	#print()
	########################-----------------------------------------------------------------------------------------------------------------------------################
	command = str.encode(os.popen("C:\\Users\\narmehran\\AppData\\Local\\Programs\\Python\\Python39\\python.exe "+"D:\\00Research\\matching\\scheduler\\TO-Upload\\c3-match-main\\real-experiments\\dataflow-load\\matchingalgo-testbed.py").read())
	output = command.decode()
	with open('D:\\00Research\\matching\\scheduler\\TO-Upload\\c3-match-main\\real-experiments\\dataflow-load\\matching-testbed.yaml', 'r') as file_read:
			...
			for key, values in dataa[ind].items():
					...
					for value in ((values)):
						for appp in range(num_of_apps):
							for micro in levels[appp]:
								#print(micro, " ", value)
								if (value == (micro)):
									#print(levels[appp].index(micro))
									#print(micro)
									alloc[appp][levels[appp].index(micro)] = resources.index(key)
									#print(resources.index(key))
	#print(":D    :D    :D")
print (alloc)